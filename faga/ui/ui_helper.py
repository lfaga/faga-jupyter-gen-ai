...
class UiHelper:
  ...
  def __init__(self, tensor_images: Any, generation_params: Any):
  ...
  def _tensor_images_to_images(self, tensor_images: torch.Tensor):
    ...
    with torch.inference_mode():
      ...
      #for timage in tensor_images:
      #  cpu_image = timage.detach().cpu()
      #  s_image = SaveImage().save_images(cpu_image, prefix, extra_pnginfo=self._generation_params)
      #  if ui := s_image.get("ui"):
      #    if results := ui.get("images"):
      #      for result in results:
      #        r_path = os.path.join("/content/ComfyUI/output", result.get("subfolder"), result.get("filename"))
      #        self._saved_images[result.get("filename")] = r_path
